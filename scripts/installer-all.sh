...
if [ -d llvm-$VERSION.src ]; then
    echo Found llvm-$VERSION.src! Not downloading source again.
else
...
if [ -d llvm-$VERSION.src/tools ]; then
    cd llvm-$VERSION.src/tools
else
    echo Fail! Something is wrong with your llvm-$VERSION.src checkout! 
fi

if [ -d clang ]; then
    echo Found clang! Not downloading clang again.
else
    $WGET http://www.llvm.org/releases/$VERSION/cfe-$VERSION.src.tar.gz
    tar xzf cfe-$VERSION.src.tar.gz
...
if [ -d $BUILD_DIR ]; then
    ...
    mkdir -p $INSTALL_DIR
    cd $BUILD_DIR
    ../llvm-$VERSION.src/configure --disable-optimized --prefix=$INSTALL_DIR
fi
...
if [ -d projects ]; then
    ...
    cd projects
    ./configure --with-llvmsrc=$CURRENT_DIR/llvm-$VERSION.src --with-llvmobj=$BUILD_DIR --prefix=`pwd`/install --disable-optimized $TARGET
